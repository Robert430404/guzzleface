...
class FileFactory
{
    ...
    public function makeFile(): File
    {
        ...
        $client = (new Objekt($this->fullyQualifiedNameSpace))
            ->extend(new Objekt($this->parent))
            ->setPhpdoc(
                (new StructurePhpdoc())
                    ->setDescription(
                        (new Description('Code Generated By Guzzleface'))
                            ->addEmptyLine()
                            ->addLine('DO NOT MODIFY DIRECTLY, ANNOTATE INTERFACE')
                            ->addEmptyLine()
                            ->addLine('@package '.$this->fullyQualifiedNameSpace)
                    )
            )
        ;
